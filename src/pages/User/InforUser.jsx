...
const thongTinUser = (thongTinNguoiDung) => {
    return <div className='h-[100vh] relative userInfo'>
        <section className="p-6 bg-orange-400 w-full md:w-[80%] lg:w-[60%] absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] rounded-lg">
...
const ketQuaDatVe = (thongTinNguoiDung) => {
    const renderTicketItem = () => {
        return thongTinNguoiDung.thongTinDatVe?.map((item, index) => {
            return <div key={index} className="p-2 lg:w-1/3 md:w-1/2 w-full ">
                <div className="bg-zinc-100 h-full flex items-center border-gray-200 border p-4 rounded-lg">
    ...
    return <div>
        <section className="text-gray-600 body-font">
            <div className="px-5 py-10 mx-auto booking_history">
                <div className="flex flex-col text-center w-full mb-10">
                    <h1 className="sm:text-3xl text-2xl font-medium title-font mb-4 text-white uppercase">lịch sử đặt vé khách hàng</h1>
                </div>
...
export default () => {
    ...
    return (
        <>
            {isLogin ? <Tabs className='pt-[6rem] min-h-[100vh] bg-zinc-100  booking' items={items} /> : <PageNotFound/>}
        </>
